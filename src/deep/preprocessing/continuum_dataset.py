...
class ContinuumDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Get the timestep images (specifically the continuum ones)
        timestep_images = sorted(f for f in all_files if f.split("__")[1].split(".")[0] == "continuum")
        timestep_images.sort(reverse=True)

        # Check if no continuum images were founds
        current_image = []
        if len(timestep_images) == 0:
            print(f"!!! NO CONTINUUM FILES FOUND for dataset_id={dataset_id}, images_path={images_path}")
        ...
        if current_image is None:
            raise ValueError(f"No valid (non-black) continuum found for sample {dataset_id}")

        # Normalize image pixels
        curr_im_arr = 1 - np.array(current_image) / 255.0               # Convert into an array with range [0,1]
    ...
    def _process_metadata(self):
        ...
        # Check if each dataset used actually has a continuum
        def has_continuum(dataset_id):
            folder_parts = dataset_id.split("_", 1)
            images_path = os.path.join(self.root_data_dir, folder_parts[0], folder_parts[1])
            ...
            for _, _, fs in os.walk(images_path):
                files.extend(fs)
            return any(f.split("__")[1].split(".")[0] == "continuum" for f in files if "__" in f)

        before = len(metadata)
        metadata = metadata[metadata["id"].apply(has_continuum)]
        after = len(metadata)
        if before != after:
            print(f"Dropped {before - after} samples with no continuum images ({after} remaining)")
