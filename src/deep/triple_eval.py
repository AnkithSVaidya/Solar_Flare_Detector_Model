...
model = TripleCNNFlareClassifier().to(device)
model.load_state_dict(torch.load("triple_cnn.pt", map_location=device))
model.eval()

# --- Load the TEST dataset ---
test_dataset = FlareDataset(features=['magnetogram', 'continuum', '304'], type='test')
...
print()
print(f"Precision (flare class): {precision:.4f}")
print(f"Recall    (flare class): {recall:.4f}")
...
def show_sample(dataset, idx, true_label, pred_label, prob, tag):
    ...
    original_cont193 = batch_data[1, :, :].cpu().numpy()
    original_cont304 = batch_data[2, :, :].cpu().numpy()
    ...
    axes[1].imshow(original_cont193, cmap='gray')
    axes[1].set_title("Continuum 193")
    axes[1].axis('off')

    axes[2].imshow(original_cont304, cmap='gray')
    axes[2].set_title("Continuum 304")
    axes[2].axis('off')
